...
def run_reset_lgw(reset_lgw_filepath, reset_lgw_pin):
    """
    ...
    LOGGER.debug("Executing %s with reset pin %s" % (reset_lgw_filepath, reset_lgw_pin_str))
...
def update_global_conf(is_sx1302, root_dir, sx1301_region_configs_dir, sx1302_region_configs_dir, region, spi_bus):
    """
    Replace global_conf.json with the configuration necessary given the concentrator
...
@retry(wait=wait_fixed(LORA_PKT_FWD_RETRY_SLEEP_SECONDS), stop=stop_after_attempt(LORA_PKT_FWD_MAX_TRIES), before_sleep=before_sleep_log(LOGGER, LOGLEVEL_INT)) # noqa
def retry_start_concentrator(is_sx1302, spi_bus, sx1302_lora_pkt_fwd_filepath, sx1301_lora_pkt_fwd_dir,
                            reset_lgw_filepath, reset_lgw_pin):
    """
    Retry to start lora_pkt_fwd for the corresponding concentrator model.
    Runs the reset_lgw script before every attempt.
    """
    run_reset_lgw(reset_lgw_filepath, reset_lgw_pin)
